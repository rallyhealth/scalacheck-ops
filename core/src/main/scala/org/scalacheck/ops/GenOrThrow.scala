...
/** Provides the ability to convert a [[Gen]] into an [[Iterator]]
  ...
  */
@deprecated(
  "Use GenFromConfig instead. This class requires a ClassTag and is not as reusable as GenFromConfig. " + "It will be removed in the next major version.",
  "2.7.0"
)
class GenOrThrow[T : ClassTag](val gen: Gen[T]) {
  ...
  /** Same as [[GenFromConfig.getOrThrow]] except throws a [[EmptyGenSampleException]]
    ...
    */
  @deprecated(
    "If you need a random sample on each call, use .nextRandom() -- " + "otherwise, if you want a stateless function, use .head instead.",
    "2.3.0"
  )
  ...
  /** Converts this generator into an infinite [[Iterator]] for a more flexible pull-based API.
    ...
    */
  @deprecated(
    "Use .iterator instead. " + "If you need to specify retries, you can pass the argument GenConfig.defaults.withRetries(n) to .iterator",
    "2.3.0"
  )
  ...
  /** Converts this generator into a lazy [[Iterable]] so that it can be converted into a collection.
    ...
    */
  @deprecated(
    "Use .iterator.to() instead. " + "If you need to specify retries, you can pass the argument GenConfig.defaults.withRetries(n) to .iterator",
    "2.3.0"
  )
